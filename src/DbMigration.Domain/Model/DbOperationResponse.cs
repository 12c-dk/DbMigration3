...
{
    public class DbValueOperationResponse<T>
    {
        public DbValueOperationResponse()
        {

        }
        public DbValueOperationResponse(T responseValue)
        {
    ...
    public class DbValueCollectionOperationResponse<T> where T : ICollection, new()
    {
        public DbValueCollectionOperationResponse() { }
        ...
        public OperationResultEnum OperationResult
        {
            get
            {
                if (OperationResponse.GeneralResponses.Any(r => r.Severity == DbOperationResponseSeverity.Error))
                {
                    return OperationResultEnum.Failure;
                }
                if (OperationResponse.GeneralResponses.Any(r => r.Severity == DbOperationResponseSeverity.Warning))
                {
                    return OperationResultEnum.PartialSuccess;
                }

                if (OperationResponse.ItemResponses.Any(r => r.Severity == DbOperationResponseSeverity.Error &&
                                                             ResponseValue.Count > 0))
                {
                    return OperationResultEnum.PartialSuccess;
                }
                ...
                if (OperationResponse.ItemResponses.Any(r => r.Severity == DbOperationResponseSeverity.Warning))
                {
                    return OperationResultEnum.PartialSuccess;
                }
        ...
        public bool IsOk
        {
            get
            {
                if (OperationResult == OperationResultEnum.Success)
                    return true;
                else
                    return false;
            }
    ...
    /// <summary>
    /// Do not use directly. Use DbValueCollectionOperationResponse&lt;T&gt; instead
    /// </summary>
    public class DbOperationResponse
    {
        ...
        public DbOperationResponse(GeneralError generalError = null, DbItemResponse item = null)
        {
            if (generalError != null)
            {
                GeneralResponses.Add(generalError);
            }
            if (item != null)
            {
        ...
        public bool IsOk
        {
            get
            {
                if (GeneralStatus == DbOperationResponseSeverity.Info && ItemStatus == DbOperationResponseSeverity.Info)
                    return true;
                else
                    return false;
            }
